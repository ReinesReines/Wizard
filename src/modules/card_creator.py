...
def create_card(card_data, output_path="card_output.png"):
    ...
    # 3. Creature image (centered, scaled 2x)
    creature_image_name = card_data.name.lower().replace(" ", "_") + ".png"
    creature_image_path = os.path.join(ASSETS_PATH, creature_image_name)
    
    if os.path.exists(creature_image_path):
        creature_img_original = Image.open(creature_image_path).convert("RGBA")
...
if __name__ == "__main__":
    ...
    # Generate cards for each creature image
    generated_count = 0
    for img_file in creature_images:
